...
<template>
    ...
    <lew-table :data="list" :columns="columns">
        ...
        <template #tags="{ row }">
            <lew-tag v-for="(item, index) in row.tags" :key="index" type="primary"> {{ item.title }}</lew-tag>
        </template>
        ...
        <template #action="{ row }">
            <lew-flex x="start" style="margin-left:-5px">
                ...
                <lew-button v-if="!row.is_publish" type="success" is-text size="small" @click="publish(row.id)">
                    发布
                </lew-button>
                <lew-button v-if="row.is_publish" type="error" is-text size="small" @click="draft(row.id)">
                    下架
                </lew-button>
                <lew-button v-if="!row.is_publish" type="error" is-text size="small" @click="del(row.id)">
                    删除
                </lew-button>
    ...
    <br>
    <lew-flex x="start">
        <lew-pagination :total="total" round :page-show-size="3" v-model:page-num="pageNum" v-model:page-size="pageSize"
            @change="getArticle"></lew-pagination>
        <div class="sub-title">
            共 {{ total }} 条记录
        </div>
    </lew-flex>
